#ifndef __PRIMATEBASE_H__
#define __PRIMATEBASE_H__
...
namespace {
    ...
    class PrimateBase : public DataFlow<BitVector>{

        public:
            static char ID;

            // set forward false in the constructor DataFlow()
            PrimateBase() : DataFlow<BitVector>(false) {
            ...
            bool isBlueCall(Instruction *ii) {
                ...
                if (isa<llvm::CallInst>(&*ii)) {
                    ...
                    //     }
                    // }
                    auto *tmp = dyn_cast<llvm::CallInst>(ii);
                    Function* foo = tmp->getCalledFunction();
                    if (!(foo->getName().contains("primate_io") || foo->getName().contains("llvm"))) {
                        res = true;
                    }
                }
                return res;
            ...
            std::vector<Value*>* getBFCOutputs(Instruction *ii) {
                if (isa<llvm::CallInst>(&*ii)) {
                    ...
                    if (call_inst->arg_size() > 1) {
                        outList->push_back(call_inst->getArgOperand(1));
                        return outList;
                    }
                }
                return NULL;
            ...
            std::vector<Value*>* getBFCInputs(Instruction *ii) {
                if (isa<llvm::CallInst>(&*ii)) {
                    ...
                    if (call_inst->arg_size() > 0) {
                        inList->push_back(call_inst->getArgOperand(0));
                        return inList;
                    }
                }
                return NULL;
                                    ...
                                    for (auto op = inList->begin(); op != inList->end(); op++) {
                                        ...
                                        if (op_type->isPointerTy()) {
                                            ...
                                            if (isa<AllocaInst>(*op)) {
                                                auto* srcOp = dyn_cast<AllocaInst>(*op);
                                                size = getTypeBitWidth(srcOp->getAllocatedType());
                                            } else if (isa<GetElementPtrInst>(*op)) {
                                                auto* srcOp = dyn_cast<GetElementPtrInst>(*op);
                                                size = getTypeBitWidth(srcOp->getResultElementType());
                                            }
                                            inOps.push_back({*op, size});
                                            memInstAddRAWDep(inst, *op, size, storeInsts);
                                        }
                                    }
                                }
                                ...
                                if (outList != NULL) {
                                    for (auto op = outList->begin(); op != outList->end(); op++) {
                                        ...
                                        if (op_type->isPointerTy()) {
                                            ...
                                            outOps.push_back({*op, size});
                                            memInstAddWARDep(inst, *op, size, loadInsts);
                                        }
                                    }
                                }
                                if (inList != NULL)
            ...
            void initializePrimateVars(Function &F) {
